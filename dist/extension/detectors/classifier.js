...
import { runRuleDetectors } from "./rules";
function clamp01(x) {
...
export function scanTextForRisk(rawText) {
    ...
    const rulesCore = 0.95 * privateKey +
        0.8 * apiKey +
        0.75 * jwt +
        0.65 * passwordAssign +
        0.75 * dbConn;
    ...
    if (explanation.length === 0) {
        ...
        if (f.containsCredentialKeywords)
            explanation.push("Credential-related language detected.");
        if (f.containsKeyValuePairs)
            explanation.push("Key/value assignment patterns detected.");
        if (f.base64Like)
            explanation.push("Base64-like encoded block detected.");
        if (f.entropyApprox > 0.7)
            explanation.push("High-entropy token-like text detected.");
    }
